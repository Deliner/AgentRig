...
`just candidate --version` builds the current product through tooling/worker/build. Its cache is `.cache/worker`; native tests always select the candidate. `just run test -- PATH` performs focused tests. Commits run the configured exported-index gate with [affected test groups](../worker/CHECKS.md); unmatched paths run the full suite. `just feature-merge` always runs full verification. `tooling/worker/run` performs no compilation. Repository configuration, memory and canonical skill sources remain versioned project policy.
...
If agent hooks use the pinned launcher, changing the pin to an uninstalled revision also prevents those hooks from running. In that case run `just bootstrap` from an ordinary terminal; the agent's PreToolUse hook can otherwise reject bootstrap itself before the recipe starts.
...
For local packaging, run `cargo +1.98.1 install --locked cargo-about --version 0.9.2 --features cli`, then `just release` after acceptance from a clean committed checkout. The CLI feature is required to install the executable. Outputs go to `.cache/distribution`. Packaging alone is not evidence that the development gate passed. The distributed binaries target Linux x86_64; releases are built on Ubuntu 24.04. A source build can target the developer's compatible Linux environment.
