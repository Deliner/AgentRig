...
def mercurial(root: Path) -> str:
    ...
    config.write_text("vcs: mercurial\n" + config.read_text())
...
def test_mercurial_delegate_reads_committed_snapshot(
...
def test_mercurial_code_result_preserves_checkout_and_applies_to_base(
    worker: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch, passes: bool
) -> None:
    ...
    assert result["code"]["vcs"] == "mercurial"
    assert source.read_text() == "uncommitted\n"
    directory = tmp_path / ".agentrig/runtime/jobs" / identifier
