...
Backlog rows contain ID, Idea and Expected benefit; BNNN links to Backlog/NNN.md with Context, Proposal and Expected benefit sections. Agents proactively use edit-backlog to save future ideas, improvements and hypotheses, then resume current work. Backlog has no execution priority. Recording or planning an idea for later does not authorize implementation; only explicit selection to execute now moves it into Plan. The memory check validates index/card consistency; older projects without Backlog remain compatible, but orphan cards are rejected. Setup preserves consumer-owned memory.
