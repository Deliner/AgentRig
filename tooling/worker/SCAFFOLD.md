...
Backlog rows contain ID, Idea and Expected benefit; BNNN links to Backlog/NNN.md with Context, Proposal and Expected benefit sections. Agents use edit-backlog to draft and agree cards in chat, saving the shown version only after explicit agreement. The skill defines business outcomes, sourced constraints and observable acceptance scenarios inside the existing sections. Backlog has no execution priority; saving does not authorize implementation. New product outcomes enter Plan only through an agreed Backlog card, with consequential questions resolved and execution and order selected. Plan references that contract and its agreed source revision; a selected set executes autonomously under execute-plan-feature. The memory check validates index/card consistency; it does not verify user agreement or semantic completeness. Older projects without Backlog remain compatible, but orphan cards are rejected. Setup preserves consumer-owned memory.

Plan holds only outcomes selected for current execution in this session or its resumed task. Rows contain ID, Status, Depends on, Feature and User capability. Details contain Feature, User capability and Acceptance; paused/complete details also contain Delivery. At most one entry is active. The agent uses edit-plan to automatically rotate completed rows/cards into Archive/Plan.md and Archive/Plan/ in the same format, preserving IDs and evidence. Dependencies may refer to current or completed archived outcomes and remain acyclic; active/complete entries require completed prerequisites. Defer future work to Backlog with its handoff context. Empty Plan is valid. The memory checker validates both indexes, cards and references; the skills guide selection and rotation without a separate scheduler.
