...
Call that MCP tool with `root`, `base`, `candidate` and optional `previous_report`. The client timeout must exceed the runner timeout plus snapshot/report overhead. Use the package README for setup or configuration errors.
