...
Apply the responsibility and change principles in
[refactor-large-directory](../refactor-large-directory/SKILL.md) when choosing
boundaries. Its semantic roles guide ownership; they do not require extra files
or directories for a local extraction.
...
5. Verify the preserved behavior and that the resulting ownership can be
   understood with less unrelated context, without following arbitrary fragments.
