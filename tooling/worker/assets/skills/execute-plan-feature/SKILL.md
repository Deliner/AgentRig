...
Run `just resume` to observe Git operations, State revision and the latest check evidence; reconcile those facts before choosing the next step. An unfinished check is not proof that a process is still running. Read agentrig.yaml for paths and Git base/prefix, then the configured State and Plan, the active feature detail if present, applicable decisions and invariants, and Git state. Reconcile State's recorded task, current VAC, verification, blockers, and next action against the actual branch, diff, and recent commits; a crash can leave it stale. Follow explicit superseding decisions. Treat Feature, User capability, Acceptance and their agreed Backlog source as the result contract; the executing agent chooses architecture and implementation. Preserve the accepted source version and criterion IDs. A feature may be a substantial MVP capability or an observable improvement such as optimization.

Plan contains only outcomes explicitly selected for execution now in the current session or resumed task. At most one feature is active. If none is active, use an existing current delivery instruction to select its ready pending entry or resume its paused one; otherwise stop. Backlog is an unordered collection of future ideas and hypotheses, never an automatic execution queue. Use edit-backlog to agree new or revised cards in chat before saving them. All new product outcomes require that source before Plan. One instruction to execute a selected set authorizes autonomous completion of its ready cards in the selected order; do not request confirmation between cards. Context reset or interruption alone does not cancel current authorized work.
...
Edit, run focused checks, and correct freely within the VAC. Resolve implementation questions from the accepted contract and existing rules, choosing the simplest sufficient solution without returning routine technical choices to the user. New validation, restrictions, normalization, formats, options and side effects require a concrete contractual or necessary technical basis; do not add behavior as a precaution. Apply the scope and acceptance rules in edit-backlog. Add or update tests when needed to verify changed behavior; a no-op or a prose edit does not require invented tests. Failed checks permit immediate correction. Inspect and stage only the coherent change, then describe the intended result, observed verification and any remaining limits in the commit message. A passing check is evidence only for the behavior it exercises. The pre-commit hook runs the configured staged-tree gate; explicit affected-path groups may select test targets. Do not routinely repeat that gate manually before committing; use `just check --only CHECK_ID` for a focused retry, adding `--staged` when checking the index. Selective test evidence does not prove the full suite passed. Integration runs the full gate; never replace either required gate with an agent-selected subset.
...
- A necessary independent prerequisite may precede a paused feature only when its agreed Backlog contract and required scope are already authorized. Add its ID to Depends on. Necessary technical work within the accepted outcome stays inside that feature; do not invent a product card to bypass agreement.
- An improvement outside current acceptance is a chat draft under edit-backlog until approved for Backlog, without delivery priority. It does not interrupt autonomous execution.
- New instructions may select additional outcomes for the current session. Record that scope and actual prerequisites, keeping one active feature. Requests to remember future work use edit-backlog's chat agreement before saving. Do not interrupt the current VAC unless the instruction changes current scope or makes that VAC obsolete.

New Plan entries require an agreed Backlog source and authorization for current execution or a prerequisite within that accepted scope. Keep stable IDs, outcome-based details and the reason/source in Delivery. An observed opportunity does not authorize a new outcome. Do not turn every implementation step into a feature or split solely because work is large.

When work reveals an improvement or hypothesis outside current acceptance, use edit-backlog to present it without saving an unapproved card, then resume the task. Recording an approved idea does not expand delivery scope. Work needed for current acceptance must still be completed or reported as a blocker.

If the accepted behavior is genuinely contradictory or requires unavailable external input, record the blocker and retained branch, continue independent authorized cards, and include unresolved items in the batch report instead of asking routine questions or guessing requirements. Do not proceed with dependent work. If no authorized work can proceed, report the concrete blocker without claiming completion.
...
Verify each agreed acceptance criterion and record its ID, met/violated/not-verified result and actual evidence in Delivery. Inspect the diff for unsupported restrictions, transformations and side effects as well as missing behavior. Follow edit-backlog for handing the same contract to an authorized independent verifier; do not add an automatic reviewer requirement. Unverified criteria cannot be reported as passed. Record actual checks and results in the relevant VAC and State. Apply edit-plan to automatically rotate completed rows and cards into Archive in the final VAC, preserving IDs, acceptance, evidence and live links. Current work may reference completed archived prerequisites. Unmet acceptance remains required work. An empty Plan is valid; after integration continue the next ready outcome from the already authorized set without another permission request.
