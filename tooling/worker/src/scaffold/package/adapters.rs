...
pub fn git_hooks() -> [(&'static str, &'static [u8]); 2] {
    [
        (".worker/hooks/pre-commit", b"#!/bin/sh\nset -eu\nroot=$(git rev-parse --show-toplevel)\n\"$root/.worker/bin/discipline-worker\" guard-commit --root \"$root\"\nexec \"$root/.worker/bin/discipline-worker\" check --root \"$root\" --staged\n"),
        (".worker/hooks/reference-transaction", b"#!/bin/sh\nset -eu\nroot=$(git rev-parse --show-toplevel)\nexec \"$root/.worker/bin/discipline-worker\" guard-reference --root \"$root\" \"$1\"\n"),
    ]
}

pub fn registration() -> Result<Vec<u8>> {
    let command = "\"$(git rev-parse --show-toplevel)/.worker/bin/discipline-worker\" hook --root \"$(git rev-parse --show-toplevel)\"";
