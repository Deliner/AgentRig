...
Completed outcomes retain their original IDs, statuses, prerequisites, acceptance and delivery evidence. This is history, not an execution queue. [Current Plan](../Plan.md) holds work selected for execution now; [Backlog](../Backlog.md) holds future ideas.

The agent rotates completed rows and cards here through edit-plan during completion, preserving references and updating architecture inventories. Never overwrite a different existing archived ID or remove a card before its archive entry is preserved.
